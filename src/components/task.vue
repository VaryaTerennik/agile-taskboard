<template>
  <div
    ...
    class="task"
    @dragstart="onDragStart($event, categoryId, index)"
    draggable="true"
    @drop="onDropTask($event, categoryId, index)"
    @dragover.prevent
    @dragenter.prevent
  >
    ...
    <div class="header-name">
      ...
      <button
        ...
        v-on:click="addToArchive($event, categoryId, index)"
      >
        AddToArchive
      </button>
      <button class="btn-remove-task" v-on:click="restoreTask($event, index)">
        RestoreTask
      </button>
    </div>
...
export default {
  ...
  methods: {
    addToArchive: function(event, categoryId, index) {
      ...
      this.$root.archive[0].tasks.push(taskIndex);
    ...
    restoreTask: function(event, index) {
      let taskIndex = this.tasks[index];
      let taskType = this.tasks[index].type;
      this.$root.archive[0].tasks.splice(index, 1);
      this.$root.categories
        .find((el) => el.id == taskType)
    ...
    onDropTask(event, toCategoryId, toTaskIndex) {
      ...
      let oToCategory = this.$root.categories.find(
        (el) => el.id == toCategoryId
      );
      let oFromCategory = this.$root.categories.find(
        (el) => el.id == fromCategoryId
      );
