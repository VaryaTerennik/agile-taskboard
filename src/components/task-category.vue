...
<template>
  <div class="categories" @drop="onDrop($event, category.id)" @dragover.prevent @dragenter.prevent>
...
export default {
  ...
  methods: {
      ...
      (this.$root.categories.find(el => el.id == this.category.id) || {}).tasks.push({name: this.newTaskName, type: this.category.id})
      fetch("http://localhost:3000/tasks", {
        method: "POST",
        mode: "cors",
        body: JSON.stringify({name: this.newTaskName, type: this.category.id})
